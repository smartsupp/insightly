...
trait Contacts
{
	...
	public function saveContactAddress($id = false, array $data = [])
	{
		...
		return empty($data['ADDRESS_ID']) ? $this->call('post', 'Contacts/' . $id . '/Addresses', $data) : $this->call('put', 'Contacts/' . $id . '/Addresses',
			$data);
	}
	...
	public function saveContactInfo(array $data = [], $id = false)
	{
		...
		return !empty($data['CONTACT_INFO_ID']) ? $this->call('put', 'Contacts/' . $id . '/ContactInfos', $data) : $this->call('post',
			'Contacts/' . $id . '/ContactInfos', $data);
	}
